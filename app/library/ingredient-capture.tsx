...
export default function IngredientCapture({user,close,done}:{user:User;close:()=>void;done:()=>void}) {
  ...
  async function lookupBarcode(code:string){setBusy("Looking up barcode…");setNotice("");try{const {data:saved,error:savedError}=await supabase!.from("ingredients").select("name,brand,barcode,serving_amount,serving_unit,calories,protein,carbohydrates,fat,source").eq("barcode",code).order("created_at",{ascending:false}).limit(1).maybeSingle();if(savedError)throw savedError;if(saved){setV({name:saved.name,brand:saved.brand||"",barcode:code,serving_amount:String(saved.serving_amount),serving_unit:saved.serving_unit,calories:String(saved.calories),protein:String(saved.protein),carbohydrates:String(saved.carbohydrates),fat:String(saved.fat),source:saved.source||"barcode"});setNotice("Loaded your saved nutrition for this barcode. You can adjust the amount before using it.");return;}const response=await fetch(`/api/barcode/${encodeURIComponent(code)}`);const body=await response.json();if(!response.ok||!body.product)throw new Error(body.error||"Product not found. Photograph the nutrition label instead.");const p=body.product,nutrition=nutritionFromProduct(p);setV(current=>({...current,name:p.product_name||current.name,brand:p.brands||current.brand,barcode:code,serving_amount:nutrition.serving_amount,serving_unit:nutrition.serving_unit,calories:nutrition.calories===null?"":String(nutrition.calories),protein:nutrition.protein===null?"":String(nutrition.protein),carbohydrates:nutrition.carbohydrates===null?"":String(nutrition.carbohydrates),fat:nutrition.fat===null?"":String(nutrition.fat),source:"barcode"}));setNotice(nutrition.found?"Barcode found and nutrition filled in. Review the values, then save the ingredient.":"This barcode is not complete in the free food database. Tap Read label once, check the values, and save it. Eats will remember it for future scans.");}catch(error){setNotice(error instanceof Error?error.message:"Could not read that product.");}finally{setBusy("");}}
