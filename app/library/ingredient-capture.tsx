...
export default function IngredientCapture({user,close,done}:{user:User;close:()=>void;done:()=>void}) {
  ...
  useEffect(()=>{if(!scanning)return;let cancelled=false;(async()=>{try{const {BrowserMultiFormatReader}=await import("@zxing/browser");const reader=new BrowserMultiFormatReader();const controls=await reader.decodeFromConstraints({audio:false,video:{facingMode:{ideal:"environment"}}},videoRef.current!,async(result,_error,activeControls)=>{if(!result||cancelled)return;cancelled=true;activeControls.stop();scannerControls.current=null;setScanning(false);await lookupBarcode(result.getText());});scannerControls.current=controls;}catch{setScanning(false);setNotice("Camera access failed. Allow camera permission in Safari settings and try again.");}})();return()=>{cancelled=true;scannerControls.current?.stop();scannerControls.current=null}},[scanning]);
  ...
  return <div className="overlay"><div className="sheet scroll-sheet"><div className="sheet-handle"/><div className="sheet-title"><div><span className="eyebrow">Food capture</span><h2>New ingredient</h2></div><button className="icon-btn" onClick={close}><X/></button></div>
    ...
    {scanning&&<div className="scanner"><video ref={videoRef} autoPlay muted playsInline/><div className="scanner-guide"><span/><p>Hold the barcode inside the frame</p></div><button type="button" onClick={()=>setScanning(false)}><X/> Cancel</button></div>}
